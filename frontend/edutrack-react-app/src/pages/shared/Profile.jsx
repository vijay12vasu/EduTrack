  ...
  return (
    <DashboardLayout title={`${userProfile.fullName}'s Profile`} subtitle={`Detailed ${userProfile.role.toLowerCase()} information`}>
     ...
     <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Profile Information</h3>
      ...
      <Input label="Name" value={userProfile.fullName} readOnly className="bg-slate-50 dark:bg-slate-900/50 " />
      <Input label="Email" value={userProfile.email} readOnly className="bg-slate-50 dark:bg-slate-900/50 " />
      <Input label="Role" value={userProfile.role} readOnly className="bg-slate-50 dark:bg-slate-900/50 " />
            
            {isStudent && (
              <>
        <Input label="Register Number" value={userProfile.registerNumber || '—'} readOnly className="bg-slate-50 dark:bg-slate-900/50 " />
        <Input label="Department" value={userProfile.department || '—'} readOnly className="bg-slate-50 dark:bg-slate-900/50 " />
        <Input label="Year" value={userProfile.year || '—'} readOnly className="bg-slate-50 dark:bg-slate-900/50 " />
              </>
            )}
       ...
       <Input label="Department" value={userProfile.department} readOnly className="bg-slate-50 dark:bg-slate-900/50 " />
            ...
            {userProfile.company && (
       <Input label="Company" value={userProfile.company} readOnly className="bg-slate-50 dark:bg-slate-900/50 " />
            )}
            {userProfile.facultyId && (
       <Input label="Faculty ID" value={userProfile.facultyId} readOnly className="bg-slate-50 dark:bg-slate-900/50 " />
            )}
          </div>
        </Card>
